...
try:
    ...
    print("Warning: 'plyer' package not found. Desktop notifications disabled.", file=sys.stderr)
...
if APP_CONFIG.get("settings", {}).get("sound_notification", False):
    try:
        ...
        # You might need placeholder sound files (e.g., work_done.wav, break_done.wav)
        # SOUND_FILE_WORK = "path/to/work_done.wav"
        # SOUND_FILE_BREAK = "path/to/break_done.wav"
    except ImportError:
        print("Warning: 'playsound' package not found, but sound notifications are enabled in config.", file=sys.stderr)
        print("Install it with: pip install playsound", file=sys.stderr)
    except Exception as e:
        print(f"Warning: Could not initialize Playsound. Sound disabled. {e}", file=sys.stderr)
...
def send_desktop_notification(title: str, message: str):
    ...
    try:
        ...
        print(f"Warning: Desktop notifications not supported on this platform by plyer.", file=sys.stderr)
    except Exception as e:
...
def play_sound_alert(session_type: str):
    ...
    if not PLAYSOUND_AVAILABLE:
        print(f"Sound Alert (playsound unavailable): {session_type} finished.") # Fallback
        return
...
if __name__ == '__main__':
    ...
    print(f"Sound Enabled in Config: {APP_CONFIG.get('settings', {}).get('sound_notification', False)}")
    ...
    if PLAYSOUND_AVAILABLE and APP_CONFIG.get('settings', {}).get('sound_notification', False):
